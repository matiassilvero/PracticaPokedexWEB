...
namespace negocio
{
    ...
    {
        //a este metodo de lista le voy a agregar un parametro opcional(string id="") esto asigna el vacio por defecto si no mando nada.
        //si mando un argumento lo guarda en ese parametro id
        public List<Pokemon> listar(string id="")//esta funcion lee registros de la BD y devuelve varios
        {
            ...
            try  //en try config todo para la conexion con la BD
            {
                ...
                if (id != "")//su el id es distinto de vacio, a la consulta de aca arriba le agrego una condicion mas. Previo a esto le agregue un espacio al final de la consulta
                {//esto me va a devolver un objeto lista con un solo elemento
        ...
        public List<Pokemon> listarConSP()
        {
            ...
            try
            {
                ...
                //datos.setearConsulta(consulta); comente este y el de arriba desde q agregue StoreProcedure
                datos.setearProcedimiento("storedListar");//de esta forma ejecuta la consulta mucho mas corta
                ...
                while (datos.Lector.Read())
                {
                    ...
                    //Para agregar esto tambien modifique mi storedProcedure, le agregue q muestre el Activo
                    aux.Activo = bool.Parse(datos.Lector["Activo"].ToString());//Lo agregue para mi funcion de inactivar
        ...
        public void agregarConSP(Pokemon nuevo)
        {
            ...
            try
            {
                ...
                datos.setearParametro("@idTipo", nuevo.Tipo.id);
                datos.setearParametro("@idDebilidad", nuevo.Debilidad.id);
                //datos.setearParametro("@idEvolucion", null); por ahora lo sacamos xq no le gusta q lo pasemos con ese null, despues hacemos una validacion o lo pasamos con un valor
                datos.ejecutarAccion();
            }
            catch (Exception ex)
            {
        ...
        {//AJUSTAMOS ESTE METODO PARA Q REACTIVE/INACTIVE A LOS POKEMONS TAMBIEN, habria q cambiarle el nombre
         //Si yo llamo a esta funcion, por defecto va desactivar el Pokemon. Si lo llamo bajo otro escenario, le envia por
         //parametro un verdadero, ahi reactiva el pokemon
            try
            {
                AccesoADatos datos = new AccesoADatos();
